...
# Railway helper (see railway.toml) — must be executable in the image.
RUN chmod 0755 /opt/hermes/docker/railway-start.sh
...
# Default config seed for docker/entrypoint.sh when /opt/data/config.yaml is missing
COPY openai-config.yaml /opt/hermes/cli-config.yaml.example
# VOLUME [ "/opt/data" ]
